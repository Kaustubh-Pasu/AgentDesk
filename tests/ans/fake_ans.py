...
class FakeANS:
        ...
        self.agents[agent_id] = {
            ...
            if endpoints is not None
            else [
                {
                    ...
                    "protocol": "A2A",
                    "transports": ["JSON-RPC"],
                    "functions": [{"id": "get_hours", "name": "Hours"}],
                },
    ...
    def _hit(self, a: dict[str, Any]) -> dict[str, Any]:
        return {
            ...
            "agentVersion": f"v{a['version']}",
            "lifecycle": {"status": a["agentStatus"]},
            "scores": {"trustScore": 70, "relevance": 1.0},
    ...
    def _handle(self, request: httpx.Request) -> httpx.Response:
        ...
        if request.url.host.startswith("transparency."):
            assert "authorization" not in request.headers, (
                "credential must never be sent to the transparency log"
            )
                ...
                "payload": {
                    "producer": {
                        "event": {
                            ...
                            "agent": {"host": agent["agentHost"], "version": f"v{agent['version']}"},
                            "attestations": {
                                "identityCert": {
                                    "fingerprint": self.identity_fingerprints.get(agent["agentId"], "")
                                }
                            },
                        }
                    }
        ...
        if path.startswith("/v1/ans/"):
            if path == "/v1/ans/search-registered-agents":
                ...
                if unsupported:
                    return httpx.Response(
                        ...
                        json={
                            ...
                            "message": f'status "{sorted(unsupported)[0]}" is not supported',
                        },
                    )
                ...
                items = [
                    ...
                    if a["agentStatus"] in statuses and (not domains or a["agentHost"] in domains)
                ]
                ...
                "dnsRecords": [
                    {
                        "name": f"_ans.{host}",
                        "type": "TXT",
                        "value": f"v=ans1; version=v{agent['version']}; p=a2a; url=https://{host}/a2a",
                        "purpose": "DISCOVERY",
                    },
                    {
                        "name": f"_ans-badge.{host}",
                        "type": "TXT",
                        "value": f"v=ans-badge1; version=v{agent['version']}; url=https://transparency.ans.godaddy.com/v1/agents/{aid}",
                        "purpose": "BADGE",
                    },
                    {
                        "name": f"_443._tcp.{host}",
                        "type": "TLSA",
                        "value": "3 0 1 " + "ab" * 32,
                        "required": False,
                    },
            ...
            if not self.dns_ok:
                return httpx.Response(
                    ...
                    json={
                        ...
                        "message": "DNS",
                        "missingRecords": [
                            {"name": f"_ans.{agent['agentHost']}", "type": "TXT", "value": "x"}
                        ],
                    },
                )
        ...
        if tail == "revoke":
            ...
            return httpx.Response(
                ...
                json={
                    ...
                    "ansName": agent["ansName"],
                    "status": "REVOKED",
                    "revokedAt": "2026-09-19T10:00:00Z",
        ...
        if tail in ("certificates/identity", "certificates/server"):
            return httpx.Response(
                200, json=self.certs.get(agent["agentId"], {}).get(tail.rsplit("/", 1)[-1], [])
            )
        return self._err(404, "NOT_FOUND")
    ...
    def _register(self, body: dict[str, Any]) -> httpx.Response:
        ...
        agent_id = self.add_active_agent(
            host,
            version,
        ...
        challenge = {
            "type": "DNS_01",
            "token": "tok-SECRETISH",
            "keyAuthorization": "tok-SECRETISH.thumb",
        ...
        pending["challenge" if self.singular_challenge else "challenges"] = (
            challenge if self.singular_challenge else [challenge]
        )
