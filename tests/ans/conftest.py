...
class FakePKI:
    ...
    def _ca(cn: str, key, issuer, issuer_key) -> x509.Certificate:  # type: ignore[no-untyped-def]
        ...
        builder = (
            x509.CertificateBuilder().subject_name(name).issuer_name(issuer.subject if issuer else name)
    ...
    def identity_cert(self, host: str, version: str = "1.0.0", *, uri: str | None = None, days: int = 90,
                      start_days_ago: int = 1) -> x509.Certificate:
        ...
        return (
            ...
            .issuer_name(self.intermediate.subject).public_key(key.public_key())
            .serial_number(x509.random_serial_number())
            .not_valid_before(now - timedelta(days=start_days_ago)).not_valid_after(now + timedelta(days=days))
            ...
            .add_extension(x509.KeyUsage(digital_signature=True, key_cert_sign=False, crl_sign=False, content_commitment=False,
                                         key_encipherment=False, data_encipherment=False, key_agreement=False,
                                         encipher_only=False, decipher_only=False), critical=True)
            .add_extension(x509.AuthorityKeyIdentifier.from_issuer_public_key(self.int_key.public_key()), critical=False)
...
def ans_settings(tmp_path):  # type: ignore[no-untyped-def]
    return make_settings(godaddy_pat=PAT, keys_dir=str(tmp_path / "keys"), artifacts_dir=str(tmp_path / "artifacts"))
