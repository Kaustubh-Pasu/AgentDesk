...
async def create_tenant(web: Web, client, label: str = "bluedoor", url: str = "https://blue-door.example.org/") -> str:  # type: ignore[no-untyped-def]
    page = await client.get("/create")
    csrf, keys = web.form_tokens(page.text)
    response = await client.post("/admin/tenants", headers=SAME, data={"csrf_token": csrf, "idempotency_key": keys[0], "display_name": "Blue Door",
                                                                       "source_url": url, "agent_label": label})
...
async def publish(web: Web, client, location: str, **fields: str) -> object:  # type: ignore[no-untyped-def]
    ...
    data = {"csrf_token": csrf, "idempotency_key": keys[0], "row_version": row_version, "business_name": "Blue Door Cafe", "description": "Coffee.",
            "category": "restaurant", "hours": "Mon-Fri | 7am - 6pm", "capabilities": ["business_information", "hours"], "confirm": "yes", **fields}
...
async def test_health_home_and_security_headers(web: Web) -> None:
    ...
    assert "Create agent" in home.text and "Find agent" in home.text and "discover → verify → communicate" in home.text
    h = home.headers
    assert h["content-security-policy"] == CSP and "'unsafe-inline'" not in CSP and "script-src 'self'" in CSP
    assert h["strict-transport-security"].startswith("max-age=63072000") and h["x-content-type-options"] == "nosniff"
    assert h["x-frame-options"] == "DENY" and h["referrer-policy"] == "no-referrer" and h["cross-origin-opener-policy"] == "same-origin"
    assert "camera=()" in h["permissions-policy"] and "server" not in h and "x-request-id" in h
    assert "<script" not in home.text and "style=" not in home.text  # nothing inline; CSP would block it anyway
...
async def test_gate1_protocols_on_both_hosts_through_the_full_stack(web: Web) -> None:
    ...
    for host, tool in ((DESK, "about_agent_desk"), (DEMO, "get_hours")):
        async with web.client(host) as client:
            ...
            a2a = await client.post("/a2a", headers={"A2A-Version": "1.0"}, json={"jsonrpc": "2.0", "id": 1, "method": "SendMessage", "params": {
                "message": {"messageId": "m", "role": "ROLE_USER", "parts": [{"text": "what can you do? help"}]}}})
            assert a2a.json()["result"]["message"]["parts"][0]["text"]
            call = await client.post("/mcp", headers=mcp_headers, json={"jsonrpc": "2.0", "id": 2, "method": "tools/call", "params": {"name": tool, "arguments": {}}})
            assert call.json()["result"]["isError"] is False


async def test_host_header_poisoning_rejected(web: Web) -> None:
    for host in ("evil.example.com", f"desk.{BASE}.evil.com", "localhost", "127.0.0.1", f"a.b.{BASE}", f"{DESK}@evil.com"):
        async with web.client(host) as client:
...
async def test_cors_is_not_open(web: Web) -> None:
    async with web.client() as client:
        preflight = await client.options("/api/find", headers={"Origin": "https://evil.example.com", "Access-Control-Request-Method": "POST"})
...
async def test_login_logout_and_session_cookie_attributes(web: Web) -> None:
    async with web.client() as client:
        ...
        cookie = next(v for k, v in response.headers.multi_items() if k == "set-cookie" and v.startswith("__Host-agentdesk_session="))
        lowered = cookie.lower()
        assert "secure" in lowered and "httponly" in lowered and "samesite=strict" in lowered and "path=/" in lowered and "domain" not in lowered
        page = await client.get("/create")
        ...
        client.cookies.set("__Host-agentdesk_session", token, domain=DESK)  # replay the old cookie after logout
...
async def test_session_fixation_is_not_possible(web: Web) -> None:
    async with web.client() as client:
        client.cookies.set("__Host-agentdesk_session", "attacker-chosen-session-token-0000000000000", domain=DESK)
    ...
    async with web.client() as victimless:
        victimless.cookies.set("__Host-agentdesk_session", "attacker-chosen-session-token-0000000000000", domain=DESK)
...
async def test_csrf_missing_bad_and_cross_origin_rejected(web: Web) -> None:
    async with web.client() as client:
        ...
        page = await client.get("/create")
        csrf, keys = web.form_tokens(page.text)
        good = {"csrf_token": csrf, "idempotency_key": keys[0], "display_name": "X", "source_url": "https://x.example.org/", "agent_label": "xlabel"}
        cases = [
    ...
    async with web.client() as anonymous:
        assert (await anonymous.post("/login", data={"email": "a@b.co", "password": "x", "csrf_token": "forged"}, headers=SAME)).status_code == 403


async def test_get_requests_never_change_state(web: Web) -> None:
    async with web.client() as client:
        await web.login(client)
        for path in ("/admin/tenants", "/logout", f"/admin/tenants/{uuid.uuid4()}/publish", f"/admin/tenants/{uuid.uuid4()}/ans/submit"):
...
async def test_create_flow_xss_inert_idempotent_and_registered(web: Web, caplog) -> None:  # type: ignore[no-untyped-def]
    ...
    async with web.client() as client:
        ...
        assert "<script>alert" not in preview.text and "&lt;script&gt;" in preview.text  # imported text is inert
        async with web.client(host) as public:
            assert (await public.get("/.well-known/agent-card.json")).status_code == 404  # not public before confirmation

        unconfirmed = await publish(web, client, location, confirm="")
        assert unconfirmed.status_code == 422
        evil = await publish(web, client, location, business_name=XSS[:100], description="{{ 7*7 }} ${7*7} <svg/onload=alert(1)>")
        assert evil.status_code == 303
        page = await client.get(location)
        assert "<script>alert" not in page.text and "<svg/onload" not in page.text and "{{ 7*7 }}" in page.text and ">49<" not in page.text
        ...
        for html in (landing.text, proof.text):
            assert "<script>alert" not in html and "onerror=alert" not in html.replace("onerror=alert(1)&gt;", "")

        # --- ANS registration through the UI: status only ever mirrors the (fake) registry
        page = await client.get(location)
        csrf, keys = web.form_tokens(page.text)
        with caplog.at_level(logging.DEBUG):
            first = await client.post(f"{location}/ans/submit", headers=SAME, data={"csrf_token": csrf, "idempotency_key": keys[2]})
            replay = await client.post(f"{location}/ans/submit", headers=SAME, data={"csrf_token": csrf, "idempotency_key": keys[2]})
        ...
        page = await client.get(location)
        assert "PENDING_VALIDATION" in page.text and f"_acme-challenge.{host}" in page.text and "acme-value-123" in page.text
        ...
        for step, expected in (("verify_acme", "PENDING_DNS"), ("verify_dns", "ACTIVE")):
            ...
            assert (await client.post(f"{location}/ans/{step}", headers=SAME, data={"csrf_token": csrf, "idempotency_key": keys[3]})).status_code == 303
    ...
    assert {"auth.login.success", "tenant.create", "import.done", "config.publish", "ans.register", "ans.active"} <= actions
...
async def test_cross_tenant_idor_and_mass_assignment(web: Web) -> None:
    async with web.client() as owner, web.client() as rival:
        ...
        assert "Blue Door" not in (await rival.get(location)).text and "Blue Door" not in (await rival.get("/create")).text
        csrf, keys = web.form_tokens((await rival.get("/create")).text)
        for action in ("import", "publish", "ans/submit", "disable"):
            data = {"csrf_token": csrf, "idempotency_key": keys[0] + action[:3], "business_name": "Hijack", "confirm": "yes", "row_version": "1",
                    "capabilities": "business_information", "password": PASSWORD}
        ...
        csrf, keys = web.form_tokens(page.text)
        await owner.post("/admin/tenants", headers=SAME, data={"csrf_token": csrf, "idempotency_key": keys[0], "display_name": "Second", "agent_label": "second",
                                                               "source_url": "https://second.example.org/", "owner_id": str(web.owners["rival"].id),
                                                               "state": "ACTIVE", "is_demo": "true", "agent_host": DESK, "role": "ADMIN"})
    async with web.services.db.session() as session:
        second = (await session.execute(select(Tenant).where(Tenant.agent_host == f"second.{BASE}"))).scalar_one()
    assert second.owner_id == web.owners["owner"].id and second.state is not TenantState.ACTIVE and second.is_demo is False


@pytest.mark.parametrize("bad_id", ["1 OR 1=1", "../../etc/passwd", "'; DROP TABLE tenants;--", "%2e%2e%2f", "0" * 400])
...
async def test_find_page_and_api(web: Web) -> None:
    ...
    async with web.client() as client:
        page = await client.post("/find", headers=SAME, data={"query": "coffee shop hours", "question": "When are you open on Saturday?"})
        api = await client.post("/api/find", headers=SAME, json={"query": "coffee shop hours", "connect": False})
        bad = await client.post("/api/find", headers=SAME, json={"query": "x", "url": "http://169.254.169.254/"})
    assert page.status_code == 200 and "Untrusted remote content" in page.text and "8:00 AM - 5:00 PM" in page.text and "15 verification checks" in page.text
    body = api.json()
    assert body["chosen"] == DEMO and body["remote_reply"] is None and body["candidates"][0]["verified"] is True
    assert bad.status_code == 422 and "169.254" not in bad.text
    sent_headers = json.dumps([h for _, _, h in web.remote.requests]).lower()
    assert "cookie" not in sent_headers and "authorization" not in sent_headers  # nothing of ours/the caller's is forwarded


async def test_proof_before_and_after_ans_active(web: Web) -> None:
    async with web.client(DEMO) as client:
        before = (await client.get("/api/proof")).json()
        assert {g["gate"]: g["status"] for g in before["gates"]} == {1: "PASS", 2: "PASS", 3: "INCOMPLETE", 4: "INCOMPLETE", 5: "FAIL"}
    ...
    assert {g["gate"]: g["status"] for g in after["gates"]} == {1: "PASS", 2: "PASS", 3: "PASS", 4: "PASS", 5: "PASS"}
    assert after["verification"]["mcp"]["probe_ok"] is True and "Gate 4" in html.text and "ACTIVE" in html.text
...
async def test_circuit_breakers_through_the_ui(tmp_path) -> None:  # type: ignore[no-untyped-def]
    async for web in make_web(tmp_path, disable_agent_creation=True, disable_remote_agent_calls=True):
        async with web.client() as client:
            ...
            response = await client.post("/admin/tenants", headers=SAME, data={"csrf_token": csrf, "idempotency_key": keys[0], "display_name": "X",
                                                                               "source_url": "https://x.example.org/", "agent_label": "xx"})
            ...
            assert (await client.get("/healthz")).status_code == 200 and (await client.get("/proof")).status_code == 200  # read-only surfaces survive
...
async def test_canary_secret_never_reaches_logs(web: Web, caplog, capsys) -> None:  # type: ignore[no-untyped-def]
    ...
    with caplog.at_level(logging.DEBUG):
        async with web.client() as client:
            await client.get("/", headers={"Authorization": f"Bearer {canary}", "Cookie": f"__Host-agentdesk_session={canary}", "X-Api-Key": PAT})
    ...
    formatted = "\n".join(h.format(r) for r in caplog.records for h in logging.getLogger().handlers if h.formatter)
