...
def make_tls_probe(address_policy: AddressPolicy, resolver: Resolver, timeout_s: float = 8.0) -> TlsProbe:
    async def probe(host: str) -> TlsEvidence:
        ...
        try:
            ...
            _, writer = await asyncio.wait_for(
                asyncio.open_connection(
                    str(addresses[0]), 443, ssl=strict_ssl_context(), server_hostname=host
                ),
                timeout_s,
            )
            ...
            return TlsEvidence(
                status="FAIL", detail=f"certificate verification failed: {exc.verify_message}"[:200]
            )
        ...
        try:
            ...
            return TlsEvidence(
                status="PASS",
                detail="hostname and chain verified against the public WebPKI",
...
class Verifier:
    def __init__(
        ...
        *,
        ...
        self._anchors = anchors or TrustAnchors.load(
            settings.ans_trust_anchor_path, settings.trust_anchor_fingerprints
        )
    ...
    async def verify(
        ...
        agent_host: str,
        *,
        candidate: DiscoveredAgent | None = None,
        ...
        if live is None:
            ...
            add(
                "canonical_agent_host",
                "Canonical agent host",
                "PASS",
                "host is a canonical public DNS name",
                observed=host,
            )
            await self._self_evidence(host, result, add)
            return self._finish(result, checks)
        result.ans = AnsEvidence(
            agent_id=live.agent_id,
            ans_name=live.ans_name,
            status=live.status,
            environment=self._ans.environment,
            checked_at=utcnow(),
        ...
        add(
            "ans_status_active",
            "ANS lifecycle is ACTIVE (live)",
            "PASS" if live.status in CONNECTABLE_STATUSES else "FAIL",
            f"registry reports {live.status or 'no status'}",
            agent_id=live.agent_id,
            ans_name=live.ans_name,
        )
        ...
        add(
            ...
            "registry agentHost and ANS name match the requested host"
            if host_ok
            else "registry host / ANS name mismatch",
            expected=host,
            observed=live.agent_host,
        )
        ...
        add(
            "endpoints_https",
            "Endpoints are HTTPS with valid TLS",
            "PASS" if https_ok and result.tls.status == "PASS" else "FAIL",
            result.tls.detail if https_ok else "a declared endpoint is not https",
            tls_version=result.tls.version,
            leaf_sha256=result.tls.leaf_sha256,
        )
        add(
            "endpoint_host_binding",
            "Endpoints are on the registered host",
            "PASS" if bound else "FAIL",
            "every endpoint/metadata URL is on the registered agentHost"
            if bound
            else "an endpoint is on a different host",
        )
    ...
    async def _consistency(
        self, live: DiscoveredAgent, candidate: DiscoveredAgent | None, add: Callable[..., Status]
    ) -> Badge | None:
        problems: list[str] = []
        incomplete: list[str] = []
        ...
        if candidate is not None and (
            candidate.agent_id != live.agent_id
            or candidate.ans_name != live.ans_name
        ...
        try:
            ...
            if (detail.agent_host.lower(), detail.ans_name, detail.status) != (
                live.agent_host.lower(),
                live.ans_name,
                live.status,
        ...
        add(
            "ans_record_consistency",
            "Registry detail and transparency log agree",
            status,
            "; ".join(problems or incomplete)
            or "search hit, agent detail and transparency-log badge are consistent",
    ...
    async def _identity(
        ...
        if not self._ans.configured:
            ...
            try:
                ...
                why = (
                    "certificate API only serves agents owned by the caller"
                ...
                add(
                    ...
                    label8,
                    "FAIL",
                    f"unparseable certificate from registry ({exc.code})",
                    mandatory=False,
                )
        ...
        if leaf is None:
            for id_, label in (
                ...
                ("identity_certificate_binding", label9),
        ...
        add(
            ...
            label8,
            "PASS",
            "retrieved from the official certificate-management API",
            mandatory=False,
        ...
        if bind_status == "PASS" and badge is not None and badge.identity_fingerprint:
            ...
            if attested != fingerprint_sha256(leaf):
                bind_status, bind_reason = (
                    "FAIL",
                    "certificate fingerprint differs from the transparency-log attestation",
                )
            else:
    ...
    async def _metadata(
        ...
        labels = (
            "Protocol metadata fetched within limits",
            "Metadata parses and matches the registration",
            ...
            try:
                ...
                if rpc_host != host or urlsplit(card.rpc_url).scheme != "https":
                    ...
                    schema_problems.append(
                        "Agent Card JSON-RPC URL differs from the ANS-registered A2A endpoint"
                    )
                result.a2a = A2AEvidence(
                    status="FAIL" if schema_problems else "PASS",
                    detail="; ".join(schema_problems) or "card valid",
                    card_url=card_url,
                    card_valid=not schema_problems,
                    card_sha256=card.sha256,
                    name=card.name,
                    version=card.version,
                    protocol_versions=list(card.protocol_versions),
                    skills=list(card.skills)[:50],
                    rpc_url=card.rpc_url,
                    signed=card.signed,
                )
            except (RemoteProtocolError, SSRFBlocked, FeatureDisabled) as exc:
                code = getattr(exc, "code", "error")
                (schema_problems if code in ("card_invalid", "invalid_json") else fetch_problems).append(
                    f"A2A card: {code}"
                )
                result.a2a = A2AEvidence(
                    status="FAIL", detail=f"Agent Card rejected: {code}", card_url=card_url
                )
            result.endpoint_checks.append(
                EndpointCheck(
                    protocol="A2A", url=a2a.agent_url, status=result.a2a.status, detail=result.a2a.detail
                )
            )
        if mcp is not None:
            try:
                session = await self._mcp.connect(mcp.agent_url)
                result.mcp = McpEvidence(
                    status="PASS",
                    detail="initialize + tools/list succeeded",
                    url=mcp.agent_url,
                    handshake=True,
                ...
                tool = next(
                    (t for t in session.tools if t.name == probe_tool and t.read_only and not t.required),
                    None,
                )
                if tool is not None:
                ...
                result.mcp = McpEvidence(
                    status="FAIL", detail=f"MCP handshake failed: {code}", url=mcp.agent_url
                )
            result.endpoint_checks.append(
                EndpointCheck(
                    protocol="MCP", url=mcp.agent_url, status=result.mcp.status, detail=result.mcp.detail
                )
            )
        add(
            "metadata_fetch",
            labels[0],
            "FAIL" if fetch_problems else "PASS",
            "; ".join(fetch_problems) or "fetched with time/size/content-type limits",
        )
        add(
            "metadata_schema",
            labels[1],
            "FAIL" if schema_problems else "PASS",
            "; ".join(schema_problems) or "parsed as data; interface matches the registration",
        )
        ...
        if card is not None and declared_hash:
            ...
            add(
                ...
                "card hash equals the ANS metaDataHash"
                if ok
            ...
            add(
                ...
                labels[2],
                "INCOMPLETE",
                "neither ANS nor the card provides a hash/signature to verify",
                mandatory=False,
            )
        return card
    ...
    async def _drift(
        ...
        if card is None or self._db is None:
            add(
                ...
                label,
                "INCOMPLETE",
                "no card to compare" if card is None else "no evidence store",
                mandatory=False,
            )
            return
        async with self._db.session() as session:
            row = (
                await session.execute(select(CardObservation).where(CardObservation.agent_host == host))
            ).scalar_one_or_none()
            now = utcnow()
            if row is None:
                ...
                status, detail = (
                    "FAIL",
                    "Agent Card changed since it was last verified: re-verification required before use",
                )
    ...
    async def _blocked(self, host: str) -> bool:
        ...
        async with self._db.session() as session:
            return (
                await session.execute(select(BlockedAgent.id).where(BlockedAgent.agent_host == host))
            ).first() is not None
    ...
    async def _self_evidence(self, host: str, result: VerificationResult, add: Callable[..., Status]) -> None:
        ...
        synthetic = [
            AnsEndpoint.model_validate(
                {
                    "agentUrl": f"https://{host}/a2a",
                    "protocol": "A2A",
                    "metaDataUrl": f"https://{host}/.well-known/agent-card.json",
                }
            ),
    ...
    def _finish(result: VerificationResult, checks: list[CheckResult]) -> VerificationResult:
        ...
        result.reasons = [
            f"{c.status}: {c.label} — {c.detail}" for c in (*failed, *open_mandatory, *open_optional)
        ][:30]
