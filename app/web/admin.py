...
def profile_from_form(form: FormData) -> BusinessProfile:
    ...
    return BusinessProfile.model_validate({
        ...
        "services": [{"name": n, "description": d, "price": p} for n, d, p in _lines(form, "services", 3, 50)],
        "menu_items": [{"name": n, "description": d, "price": p, "section": s} for n, d, p, s in _lines(form, "menu_items", 4, 100)],
...
def profile_to_form(profile: dict[str, Any]) -> dict[str, str]:
    ...
    return {"business_name": profile.get("business_name", ""), "description": profile.get("description", ""),
            "category": profile.get("category", "other"), "address": profile.get("address", ""),
...
def register_admin(app: FastAPI, services: Services, render: Render) -> None:
    ...
    async def guarded_form(request: Request, principal: Principal, operation: str, payload: Any) -> tuple[FormData, idempotency.IdempotencyOutcome]:
        form = await request.form()
        await require_csrf(request, principal)
        outcome = await idempotency.begin(services.db, actor_id=principal.id, operation=operation,
                                          key=str(form.get("idempotency_key", "")), payload=payload)
        return form, outcome
    ...
    async def create_page(request: Request) -> Response:
        ...
        return render(request, "create.html", tenants=await services.tenants.list_for(principal.user.id),
                      csrf_token=principal.session.csrf_token, idempotency_key=idempotency.new_idempotency_key(),
    ...
    async def tenant_page(request: Request, tenant_id: str) -> Response:
        ...
        async with services.db.session() as session:
            registration = (await session.execute(select(ANSRegistration).where(
                ANSRegistration.tenant_id == view.tenant.id).order_by(ANSRegistration.created_at.desc()))).scalars().first()
        shown = view.draft or view.published
        return render(request, "tenant.html", tenant=view.tenant, has_draft=view.draft is not None, published=view.published,
                      form=profile_to_form(shown.profile) if shown else None, selected=set(shown.allowed_capabilities) if shown else set(),
                      capabilities=[c.value for c in Capability], categories=[c.value for c in BusinessCategory], registration=registration,
                      origin=settings.origin_for(view.tenant.agent_host), csrf_token=principal.session.csrf_token,
                      keys=[idempotency.new_idempotency_key() for _ in range(8)], ans_configured=settings.ans_credential_configured or bool(settings.controlplane_url),
    ...
    async def tenant_create(request: Request) -> Response:
        ...
        data = TenantCreateInput(display_name=str(raw.get("display_name", "")), source_url=str(raw.get("source_url", "")),
        ...
        except Exception as exc:  # the tenant exists; the owner sees the import error on its page and can retry
    ...
    async def tenant_publish(request: Request, tenant_id: str) -> Response:
        ...
        data = ProfileConfirmInput(profile=profile_from_form(raw), capabilities=[Capability(c) for c in raw.getlist("capabilities") if c in {x.value for x in Capability}],
                                   row_version=int(row_version) if row_version.isdigit() else 0, confirm=raw.get("confirm") == "yes")
        _, outcome = await guarded_form(request, principal, "tenant.publish", {"tenant": str(tid), "data": data.model_dump(mode="json")})
        if not outcome.replayed:
            try:
    ...
    async def logout(request: Request) -> Response:
        ...
        async with services.db.session() as session:
            ...
            await audit.record(session, action=audit.LOGOUT, outcome="ok", actor_type="user", actor_id=principal.id)
