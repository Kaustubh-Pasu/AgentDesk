...
def render(request: Request, template: str, *, status: int = 200, **context: Any) -> HTMLResponse:
    ...
    base = {
        "settings_view": {
            "desk_origin": services.settings.desk_origin,
            "demo_origin": services.settings.origin_for(services.settings.demo_host),
            "desk_host": services.settings.desk_host,
            "demo_host": services.settings.demo_host,
...
def error_response(
    ...
    response = render(
        ...
        code=code,
        message=message,
        title=_STATUS_TEXT.get(status, "Error"),
    )
...
def register_web(app: FastAPI, services: Services) -> None:
    ...
    async def _forbidden(request: Request, exc: Forbidden) -> Response:
        await audit.record_independent(
            services.db,
            action=audit.CSRF_REJECTED if exc.code.startswith("csrf") else audit.AUTHZ_DENIED,
            outcome="denied",
            actor_type="ip",
            actor_id=client_ip(request, settings),
            metadata={"code": exc.code, "path": request.url.path},
        )
    ...
    async def _ans(request: Request, exc: AnsApiError) -> Response:
        return error_response(
            ...
            502,
            "ans_" + exc.code.lower(),
            "GoDaddy ANS did not accept the request: " + (exc.message or exc.code),
        )
    ...
    async def home(request: Request) -> Response:
        ...
        request.state.principal = (
            await current_session(request, services.db, settings) if agent.kind == "desk" else None
        )
        if agent.kind == "desk":
    ...
    async def mcp_json(request: Request) -> Response:
        agent = await served_agent(request)
        return JSONResponse(
            mcp_metadata(agent.kind, agent.host, settings), headers={"Cache-Control": "public, max-age=60"}
        )
    ...
    async def find_page(request: Request) -> Response:
        await desk_only(request)
        return render(
            request, "find.html", outcome=None, form={"query": "", "exact_host": "", "question": ""}
        )
    ...
    async def find_submit(request: Request) -> Response:
        ...
        data = FindInput(
            query=values["query"], exact_host=values["exact_host"], question=values["question"], connect=True
        )
    ...
    async def login_page(request: Request) -> Response:
        ...
        holder = Response()
        token = csrf.issue_login_csrf(
            holder, settings
        )  # signed double-submit token bound to a fresh nonce cookie
    ...
    async def login_submit(request: Request) -> Response:
        ...
        async with services.db.session() as session:
            user = (
                (await session.execute(select(User).where(User.email == email))).scalar_one_or_none()
                if data
                else None
            )
            ok = verify_password(
                user.password_hash if user else None, data.password if data else secrets.token_urlsafe(8)
            )
            ok = bool(ok and user is not None and user.disabled_at is None)
            if not ok:
                await services.limiter.hit(
                    "login-fail", who, limit=settings.rl_login_failures, window_s=settings.rl_login_window_s
                )
                await audit.record(
                    session, action=audit.LOGIN_FAILURE, outcome="denied", actor_type="ip", actor_id=ip
                )
                ...
                response = render(
                    ...
                    csrf_token=csrf.login_token_for(request, settings) or "",
                )
                return response
            ...
            if old is not None:
                await sessions.destroy_session(
                    session, old
                )  # never keep a pre-login session alive (fixation)
            token, _row = await sessions.create_session(
                session, settings, user, request.headers.get("user-agent", "")
            )
            await audit.record(
                session, action=audit.LOGIN_SUCCESS, outcome="ok", actor_type="user", actor_id=str(user.id)
            )
            await session.commit()
        await services.limiter.reset("login-fail", who)
        redirect = RedirectResponse("/create", status_code=303)
        sessions.set_session_cookie(redirect, settings, token)
        redirect.delete_cookie(
            csrf.LOGIN_CSRF_COOKIE, path="/", secure=True, httponly=True, samesite="strict"
        )
        return redirect
